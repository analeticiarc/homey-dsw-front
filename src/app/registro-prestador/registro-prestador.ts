...
@Component({
  selector: 'app-registro-prestador',
  standalone: true,
  imports: [CommonModule, FormsModule],
  templateUrl: './registro-prestador.component.html',
  styleUrl: './registro-prestador.component.css'
})
export class RegistroPrestador {
  ...
  async buscarCoordenadas() {
    ...
    try {
      ...
      console.error('[RegistroPrestador] Erro ao buscar coordenadas', error);
      this.errorMessage =
        error?.response?.data ??
  ...
  async onSubmit() {
    ...
    try {
      const response = await api.post('/usuario/prestador', payload);
